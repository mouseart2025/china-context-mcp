...
**二、目录收录仍是 0（实测，带假词对照）。** 扫描了 12 个 MCP 目录/市场：

| 站点 | 实测结果 | 判定依据 |
|---|---|---|
| mcp.so | `No servers match "china-context-mcp" / 0 results` | **0 条结果** |
| Smithery | 命中仅在 `{"q":"?q=china-context-mcp"}` 里 | 页面**查询串自回显** |
| PulseMCP | 命中仅在 `<input value="…">` 里 | **搜索框自回显** |
| Glama / LobeHub / vibehackers / FastMCP / mcpmarket / mcpservers.org / mcp.directory | UNDECIDED | 均为 SPA，HTML 里判不出，需开浏览器看 |

> 这一步踩了**第八个坑**：把查询词填进搜索框，再抓检索结果页做子串匹配，
> 「命中」必然出现在搜索框 value、筛选链接的 `?q=`、乃至「0 条结果」的提示语里 ——
> 朴素实现会报「已在 3 家目录收录」，实际 0 家。
> 修法：只匹配可见正文 + 排除回显形态 + 跑一个假词对照。
...
- 官方 Registry 已发布，`status=active`

**四、仍需人工的**：Glama 认领（凭仓库内一个知名文件验所有权）、mcp.so / LobeHub 表单提交。
这三家规则各不相同，没有统一入口。
